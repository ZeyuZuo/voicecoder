...
- [x] 4.1 移除“只显示最后 5 条事件”的限制。
- [x] 4.2 使用滚动时间线；默认跟随最新事件，用户上滚后暂停自动滚动。
- [x] 4.3 顶部显示运行状态、耗时、文件数、总 additions / deletions。
- [x] 4.4 计划使用结构化步骤列表显示 pending / inProgress / completed。
- [x] 4.5 agent commentary 作为过程说明流式显示。
- [x] 4.6 文件修改按文件聚合，不为每个 patch 重复生成卡片。
- [x] 4.7 命令卡片支持展开完整输出、复制命令和复制错误。
- [x] 4.8 diff 默认只显示统计和摘要，展开后显示当前 diff。
- [x] 4.9 warning 使用黄色非阻塞提示，terminal error 使用红色失败卡片。
- [x] 4.10 增加“最后进展于 N 秒前”，超过阈值时显示“仍在等待 Codex”。
- [x] 4.11 运行结束后保留完整时间线，不被 preview 状态覆盖。
...
目标：出现版本变化、崩溃或页面刷新时仍可通过日志排查，同时每次启动保持全新页面。
...
- [x] 7.5 DemoSession 快照仅作为本地审计日志，应用启动和刷新时不读取或恢复。
...
- DemoSession 使用 schema v2 原子快照，保存 `itemsById`、`itemOrder`、文件、计划、warning/error、runtime 和 `codexThreadId`；并阻止旧的异步保存覆盖较新状态。这些快照只用于离线排查，不参与应用状态初始化。
- 页面启动和刷新时不读取项目、对话或 DemoSession 历史状态；React 状态始终从空项目、空对话和空工作区开始。
...
- Rust `cargo check` 与 145 个非 ignored 测试通过；3 个依赖真实账号/服务的 smoke 测试保持 ignored。
...
- [x] 8.11 新建一个包含多个文件的前端 Demo。
- [x] 8.12 连续修改同一个文件多次，确认页面只更新一张文件卡片。
- [x] 8.13 同一轮新增、修改、删除和移动文件。
- [x] 8.14 运行成功、失败和长时间无输出的命令。
- [x] 8.15 触发一次自动审批批准和一次自动审批拒绝。
- [x] 8.16 模拟可重试网络错误和最终失败。
- [x] 8.17 模拟未知 notification，确认主流程继续运行。
- [x] 8.18 运行期间刷新页面，确认回到全新页面且不恢复时间线。
- [x] 8.19 Agent 完成后自动启动 dev server 并打开 preview。
