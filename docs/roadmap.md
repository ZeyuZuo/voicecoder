...
- 系统按时间间隔或批量阈值周期性总结用户已经说过的内容，并在页面展示“当前理解”。
- 需求整理状态：
  - 原始输入。
  - 当前理解。
  - 待澄清问题。
...
- 用户点击“我说完了”后，AI 生成需求文档草稿并主动追问关键不明确点。
- 用户继续用语音回答澄清问题，回答进入同一个需求会话；`clarifying -> processing -> clarifying` 可以循环，直到 AI 判断需求明确。
- 用户确认需求文档后生成 Coding Prompt。

验收标准：

- 语音输入不会只停留在转写文本，而是能被整理成结构化需求。
- 进入语音需求采集模式后，不再依赖普通 prompt 文本框输入需求。
- 待确认阶段禁用麦克风，用户确认后才生成需求文档。
- 不明确需求不会直接触发编码。
