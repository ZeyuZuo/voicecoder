...
- 点击语音按钮后进入语音专用需求采集模式，普通文本输入不再作为本轮需求入口。
- 转写句子按时间顺序进入对话上下文。
- 多人语音结果可以保留 provider 返回的 speaker 信息，但默认不强依赖 speaker 准确性。
- 系统按安静期、批量阈值或最长等待时间实时理解用户已经说过的内容，并在页面小云朵中展示“当前理解”和最多 3 个关键缺口。
- 需求整理状态：
  - 原始输入。
  - 当前理解。
  - 实时缺口提示。
  ...
  - 验收标准。
  - 完整需求文档。
- AI 通过真实 OpenAI-compatible API 总结当前需求。
- 用户继续自然说话时，AI 自动识别缺口是否已被补充，并在小云朵中消除已解决提示。
- 用户最后只点击一次“我说完了”，AI 停止录音后生成完整需求文档并写入项目 `.voicecoder` 目录。
- 用户确认需求文档后可生成 Coding Prompt。

验收标准：

- 语音输入不会只停留在转写文本，而是能被整理成结构化需求。
- 进入语音需求采集模式后，不再依赖普通 prompt 文本框输入需求。
- `finalizing`、`document_ready` 和 `confirmed` 阶段禁用麦克风。
- 不明确需求不会直接触发编码；如果用户点击“我说完了”时仍有缺口，需求文档必须写明未明确项和默认假设。
- 用户可以确认整理后的需求文档和 Coding Prompt。
- Phase 3 使用真实 OpenAI-compatible API，不实现 mock LLM provider。

## Phase 4：Demo Agent Loop MVP

目标：跑通“确认需求后生成第一版 demo，并自动启动 dev server、在右侧浏览器展示 demo”的首次演示闭环。语音反馈修改保留为后续增强，不再阻塞当前 Phase 4 主线。

核心判断：

- Phase 4 不再只设计成一次性 `codex exec` 调用。
- 第一版正式主线优先接入 `codex app-server`，先完成从需求文档到可展示 demo 的首次生成。
- `codex exec --json` 保留为诊断、实验和后备 provider，不作为长期交互主路径。
- Phase 3 产物中的需求文档和 Coding Prompt 作为 DemoSession 的初始输入。
- 第一轮是 `initial_build`；后续语音反馈修改仍按 `feedback_change` 设计，但顺延到 preview 闭环之后。
...
- 自动运行项目 dev server，例如 `npm run dev`。
- 解析 localhost preview URL。
- 右侧浏览器标签页打开 demo。
- 语音反馈修改链路后置。
...
- 用户能看到 Agent 正在做什么。
- 第一版 demo 完成后，系统自动启动 dev server。
- 右侧浏览器标签页能自动打开 demo URL。
- 每轮执行前必须确认目标项目、运行类型和本轮 prompt。
- 失败时保留可排查日志。
- 完成后刷新文件树，并进入可展示 preview 状态。
...
目标：在 Phase 4 最小预览闭环基础上，补齐更完整的预览、终端和 dev server 管理体验。
